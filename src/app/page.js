...
const LandingPage = () => {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            // Dropdown removal means we don't need this listener if we remove portals
        };
        // Removed for simplicity as portals are gone
    }, []);
            ...
            <nav className="fixed w-full z-[100] bg-white/95 backdrop-blur-md border-b border-sky-100">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between h-20 items-center">
                        ...
                        <div className="hidden md:flex items-center space-x-8">
                            <a href="https://wa.me/2348122646941" target="_blank" rel="noopener noreferrer" className="text-slate-600 hover:text-sky-600 font-bold text-sm transition-colors">Admission</a>
                            <Link href="/aboutUs" className="text-slate-600 hover:text-sky-600 font-bold text-sm transition-colors">About Us</Link>
                            <Link href="/elolamServices" className="text-slate-600 hover:text-sky-600 font-bold text-sm transition-colors">Services</Link>
                            <Link href="/donate" className="flex items-center gap-2 px-5 py-2 bg-rose-50 text-rose-600 rounded-full font-black text-xs uppercase tracking-wider hover:bg-rose-500 hover:text-white transition-all duration-300 shadow-sm hover:shadow-rose-100 group">
                ...
                {isOpen && (
                    <div className="md:hidden bg-white border-t border-sky-50 shadow-2xl absolute w-full left-0 p-6 flex flex-col space-y-4 animate-in slide-in-from-top duration-300">
                        <a href="https://wa.me/2348122646941" target="_blank" rel="noopener noreferrer" className="text-lg font-bold text-slate-700 border-b border-slate-50 pb-2" onClick={() => setIsOpen(false)}>Admission</a>
                        <Link href="/aboutUs" className="text-lg font-bold text-slate-700 border-b border-slate-50 pb-2" onClick={() => setIsOpen(false)}>About Us</Link>
                        <Link href="/elolamServices" className="text-lg font-bold text-slate-700 border-b border-slate-50 pb-2" onClick={() => setIsOpen(false)}>Services</Link>
                        <Link href="/donate" className="flex justify-between items-center text-lg font-black text-rose-500 py-2 border-b border-slate-50" onClick={() => setIsOpen(false)}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12">
                        ...
                        <div className="flex flex-col md:items-end">
                            ...
                            <div className="space-y-4 text-sm font-medium">
                                <Link href="/donate" className="block text-rose-400 font-bold hover:text-rose-300 transition-colors">Support Our Mission</Link>
                                <p className="flex items-center gap-3 md:flex-row-reverse"><MapPin size={18} className="text-sky-500" /> Lagos & Ogun State</p>
                                <a href="https://wa.me/2348122646941" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 md:flex-row-reverse hover:text-sky-400 transition-colors">
                                    <Phone size={18} className="text-sky-500" /> +234 812 264 6941
                                </a>
                                <div className="flex gap-4 md:flex-row-reverse pt-2">
                                    <a href="https://www.facebook.com/share/1BCNy868Nm/?mibextid=LQQJ4d" target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-900 rounded-full hover:bg-sky-600 hover:text-white transition-all" title="Facebook">
                                        <Facebook size={18} />
                                    </a>
                                    <a href="https://www.instagram.com/el_olamspecialhome?igsh=amRtZHB1OHB0em9p&utm_source=qr" target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-900 rounded-full hover:bg-sky-600 hover:text-white transition-all" title="Instagram">
                                        <Instagram size={18} />
                                    </a>
                                </div>
                            </div>
